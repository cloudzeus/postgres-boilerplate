...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await extractTaxForm(buf, mime, [{
      fieldKey: key,
      label: body.label,
      aiHint: body.aiHint ?? null,
      regionHint: body.regionHint,
      valueType: body.valueType as FinancialValueTypeStr,
    }]);
    const raw = result.values[key] ?? null;
    const value = coerceFinancialValue(raw, body.valueType as FinancialValueTypeStr);
    return NextResponse.json({ raw, value, model: result.model });
  } catch (err: any) {
