...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ name: result.name, code: result.code, columns: result.columns, rows: result.rows, headers: result.headers, grid: result.grid, model: result.model });
  } catch (err: any) {
