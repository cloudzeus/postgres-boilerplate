...
const Body = z.object({ page: z.number().int().min(0), mode: z.enum(['marks', 'all']).default('all'), takenKeys: z.array(z.string().max(60)).max(100).default([]) });
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { page, mode, takenKeys } = parsed.data;
  ...
  const max = Math.max(0, COLOR_PALETTE.length - t.fields.length);
