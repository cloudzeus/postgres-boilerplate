...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const file = await prisma.mediaFile.update({ where: { id: params.id }, data: parsed.data });
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  ...
  const ok = await deleteMediaFile(params.id);
  if (!ok) return NextResponse.json({ error: 'not_found' }, { status: 404 });
  await logAudit({ userId: actor.id, userEmail: actor.email, action: 'media.delete', resource: 'media', resourceId: params.id });
