...
- Scope v1: **ρύθμιση (configuration) + οθόνη κατανάλωσης (consuming view)** — φιλτραρισμένο checklist
  ανά εταιρία × πρόγραμμα.
...
### 3.5 Consuming view — `Required documents` ανά εταιρία × πρόγραμμα
Στην οθόνη εταιρίας-σε-πρόγραμμα (καρτέλα `CompanyAssessment` detail): δείχνει τον **τύπο** της
εταιρίας και, ομαδοποιημένα ανά φάση, **μόνο** τα requirements που περνούν το `requirementApplies`
(βλ. §1.3), με ένδειξη υποχρεωτικό/προαιρετικό. Αν `company.businessTypeId == null` → banner «η
εταιρία δεν έχει χαρτογραφημένο τύπο — όρισέ τον» + link στη φόρμα εταιρίας. (Η ακριβής σελίδα/route
του consuming view επιβεβαιώνεται στο plan με βάση το πού ζει σήμερα η σχέση εταιρία↔πρόγραμμα.)
...
4. **Consuming view**: φιλτραρισμένο required-documents checklist + warnings.
