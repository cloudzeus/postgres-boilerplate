/**
 * Επόμενος ΕΛΕΥΘΕΡΟΣ κωδικός μητρώου — καθαρή λογική, χωρίς I/O, ΧΩΡΙΣ γνώση του τι αριθμείται.
 *
 * Γιατί υπάρχει: το `CODE` είναι `required: true`, `calculated: false`, `defaultvalue: ""` και
 * στους συναλλασσομένους (`TRDR`: SUPPLIER / CREDITOR / DEBTOR) και στα μητρώα ειδών
 * (`MTRL` 51/52/53, `EXPN`). Η αυτόματη αρίθμηση ΔΕΝ είναι ιδιότητα του object — θα ήταν μάσκα
 * αρίθμησης της εγκατάστασης, και αυτή η εγκατάσταση δεν έχει τέτοια: `setData` με το `CODE`
 * **παραλειμμένο** απορρίπτεται με «Δεν έχετε συμπληρώσει το πεδίο \'Κωδικός\'»
 * (επιβεβαιωμένο ζωντανά σε συναλλασσόμενο, 2026-09-16).
 *
 * Άρα τον κωδικό πρέπει να τον δώσει κάποιος. Αντί να τον εφεύρουμε, τον
 * **συμπεραίνουμε από ό,τι ήδη υπάρχει** στο ΙΔΙΟ μητρώο — προμηθευτές από προμηθευτές,
 * υπηρεσίες από υπηρεσίες, έξοδα από έξοδα. Ποτέ από περιγραφή, ποτέ από άλλο μητρώο.
 */
...
export type CodeSource =
  /** Συμπεράθηκε από τους υπάρχοντες κωδικούς του ΙΔΙΟΥ μητρώου. */
  | 'pattern'
...
/**
 * Είναι ο `candidate` ΠΙΑΣΜΕΝΟΣ μέσα στο `existing`; Η σύγκριση αγνοεί κεφαλαία/πεζά και κενά
 * στα άκρα, όπως και η ίδια η {@link nextCode} — ένα «ΑΒ-1» δεν είναι ελεύθερο επειδή το μητρώο
 * το γράφει «αβ-1».
 *
 * Χρησιμεύει όταν ΥΠΑΡΧΕΙ ήδη υποψήφιος κωδικός (π.χ. ο κωδικός του προμηθευτή πάνω στη γραμμή
 * του τιμολογίου) και το μόνο ερώτημα είναι αν χωράει.
 */
export function isCodeTaken(existing: readonly string[], candidate: string): boolean {
  const c = String(candidate ?? '').trim().toUpperCase();
  if (!c) return false;
  return existing.some((e) => String(e ?? '').trim().toUpperCase() === c);
}

const MAX_PROBE = 10_000;

/**
 * Ο επόμενος ελεύθερος κωδικός για ΕΝΑ μητρώο.
 *
 * Αλγόριθμος:
 ...
 * 5. Χωρίς κανέναν κωδικό, πέφτουμε στη **μάσκα** του μητρώου (αν έχει οριστεί) —
 ...
 *
 * Το `existing` πρέπει να περιέχει κωδικούς **ΜΟΝΟ** του ίδιου μητρώου: ένας πιστωτής
 * δεν κληρονομεί την αρίθμηση των προμηθευτών ή των λογαριασμών τραπέζης, και μια υπηρεσία
 * (MTRL 52) δεν κληρονομεί την αρίθμηση των ειδών (MTRL 51).
 */
