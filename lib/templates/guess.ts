...
const CURRENCY_MARK_RE = /€|\beur\b|ευρ(ώ|ω)/i;
// Space-grouped numbers like "1 234,56" are deliberately left as TEXT for the user to retype.
...
const GL_ACCOUNT_RE = /^\d{2}(\.\d{2,3}){2,}$/;
/** dd.mm.yy(yy) shape — used to keep short dotted dates from being swallowed by the GL check. */
const SHORT_DOT_DATE_RE = /^(\d{1,2})\.(\d{1,2})\.(\d{2}|\d{4})$/;

function looksLikeCalendarDate(s: string): boolean {
  const m = SHORT_DOT_DATE_RE.exec(s);
...
export function guessValueType(raw: string): TemplateValueType {
  ...
  if (!s) return 'TEXT';
  if (!looksLikeCalendarDate(s) && isGlAccount(s)) return 'TEXT';
  if (coerceValue(s, 'DATE') !== null) return 'DATE';
