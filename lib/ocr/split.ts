...
export function documentNumberOf(text: string): string | null {
  const m = DOC_NUMBER.exec(normalizeForSplit(text));
  if (!m) return null;
  const raw = m[1].replace(/[^\p{Lu}\p{N}]/gu, '');
  // Ένας «αριθμός» χωρίς κανένα ψηφίο δεν είναι αριθμός παραστατικού (π.χ. «ΑΡ. ΛΟΓΑΡΙΑΣΜΟΥ ΙΒΑΝ»).
  return /\p{N}/u.test(raw) ? raw : null;
}
