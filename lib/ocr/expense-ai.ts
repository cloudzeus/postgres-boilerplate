...
export async function suggestExpensesWithAi(input: {
  ...
  if (strategy === 'two-stage' && sound.length) {
    ...
    // Το ιστορικό του εκδότη ΔΙΕΥΡΥΝΕΙ τη λίστα, δεν τη στενεύει: ένδειξη, όχι απόφαση.
    for (const g of fresh) history.get(g.afm)?.branches.forEach((b) => allowed.has(b) && chosen.add(b));
    grounded = articlesInBranches(sound, chosen);
  }
