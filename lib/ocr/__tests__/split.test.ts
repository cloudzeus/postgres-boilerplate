...
describe('ανιχνευτές σελίδας', () => {
  ...
  it('η ετικέτα είναι ΑΥΤΟΤΕΛΗΣ λέξη — το «ΑΡ» μέσα στο «ΠΑΡΑΛΗΠΤΗΣ» δεν μετράει', () => {
    // Χωρίς lookbehind, το «ΑΡ» του «ΠΑΡΑΛΗΠΤΗΣ» έπιανε το «ΑΛΗΠΤΗΣ» ως αριθμό (χωρίς ψηφίο) και
    // σκίαζε τον πραγματικό αριθμό — δηλαδή το τμήμα δεν έσπαγε ποτέ εκεί που έπρεπε.
    expect(documentNumberOf('ΠΑΡΑΛΗΠΤΗΣ DGESPA AE Αριθμός: 4407')).toBe('4407');
    expect(documentNumberOf('ΠΑΡΑΣΤΑΤΙΚΟ ΠΩΛΗΣΗΣ ΑΡΙΘ. 88')).toBe('88');
    expect(documentNumberOf('ΠΑΡΑΛΗΠΤΗΣ DGESPA AE')).toBeNull();
  });

  it('προσπερνάει ετικέτα που δεν ακολουθείται από αριθμό', () => {
    expect(documentNumberOf('ΑΡ. ΛΟΓΑΡΙΑΣΜΟΥ ΙΒΑΝ Αριθμός: 17')).toBe('17');
  });
